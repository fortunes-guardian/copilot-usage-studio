...
export function createLocalRuntime(options = {}) {
  ...
  function refresh(reason = 'manual') {
    ...
    scanProgress = {
      stage: 'starting',
      message: `Starting ${reason} scan.`,
      updatedAt: lastScanStartedAt,
    };
    emitLog('log', `Starting local data refresh (${reason}).`);
    ...
    activeScan = Promise.resolve()
      ...
      .then((nextSessionData) => {
        ...
        scanProgress = {
          stage: 'complete',
          message: `Imported ${nextSessionData.sessions.length} sessions.`,
      ...
      .catch((error) => {
        ...
        emitLog('error', `Local data refresh failed: ${lastError}`);
